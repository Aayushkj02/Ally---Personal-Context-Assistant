...
describe('Migration 004: Preference Unique', () => {
  ...
  it('M41-1: Newest row survives', async () => {
    ...
    await db.runAsync(
      ...
      ['pref_old', 'profile_study', 'brightness', encodeValue(40), 'user', 'set brightness to 40', 1000]
    );

    // Row B
    await db.runAsync(
      'INSERT INTO preference (id, profileId, capability, value, source, sourceCommand, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
      ['pref_new', 'profile_study', 'brightness', encodeValue(30), 'user', 'set brightness to 30', 2000]
    );

    await migration004(db);

    const rows = await db.getAllAsync<any>('SELECT * FROM preference WHERE profileId = ? AND capability = ?', ['profile_study', 'brightness']);
  ...
  it('M41-3: Unique index exists after cleanup', async () => {
    ...
    await expect(
      db.runAsync(
        'INSERT INTO preference (id, profileId, capability, value, source, sourceCommand, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ['pref_duplicate_after_migration', 'profile_study', 'brightness', encodeValue(30), 'user', null, 2000]
      )
  ...
  it('M41-4: Different natural keys survive', async () => {
    ...
    const studyBright = rows.find(r => r.profileId === 'profile_study' && r.capability === 'brightness');
    ...
    const sleepBright = rows.find(r => r.profileId === 'profile_sleep' && r.capability === 'brightness');
