...
function extractSchedule(
  ...
  const kind = text.includes('weekday')
    ? 'weekdays'
    : 'once';
...
function extractRequestedChanges(
  text: string,
  ...
  // Ringer
    ...
    text.includes('ringer normal') ||
    text.includes('normal ringer')
  ) {
    changes.push({
      capability: 'ringer',
      value: 'normal',
  ...
  if (
    ...
    text.includes('alarms only') ||
    text.includes('alarm only')
  ) {
    changes.push({
      capability: 'dnd',
      value: 'alarms_only',
    ...
    text.includes('total silence') ||
    text.includes('total silence mode')
  ) {
    changes.push({
      capability: 'dnd',
      value: 'total_silence',
  ...
  const brightnessMatch = text.match(
    /brightness(?:\s+to|\s+at|\s+of)?\s*(\d{1,3})\s*%?/i,
  );
  ...
  if (brightnessMatch) {
    ...
    if (alarmMatch) {
        ...
        const alarmTime = `${String(hour).padStart(2, '0')}:${String(
        minute,
        ).padStart(2, '0')}`;
